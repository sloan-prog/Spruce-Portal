...
export default async function handler(req, res) {
  ...
  try {
    ...
    const row = {
      submission_id:   String(body.submissionID || ''),
      submission_date: new Date().toISOString(),
      property_id:     String(raw.propertyid   || raw.property_id   || ''),
      property_name:   String(raw.propertyname || raw.property      || ''),
      bedrooms:        Number(raw.bedrooms)     || null,
      bathrooms:       Number(raw.baths)        || null,
      sleeps:          Number(raw.sleeps)        || null,
      plan_type:       String(raw.plantype      || ''),
      coffee_enabled:  String(raw.coffeeenabled || ''),
      coffee_type:     String(raw.coffeetype    || ''),
    ...
    // Decrement property_stock for each item set out
    if (row.property_id) {
      const deductions = [
